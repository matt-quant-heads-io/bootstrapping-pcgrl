...
def convert_state_to_onehot_encoded(integer_based_state):
    one_hot_encoded_state = []
    for row_idx, row_i in enumerate(integer_based_state):
        new_row = []
        for col_idx, col_i in enumerate(row_i):
            new_cell = [0]*8
            new_cell[integer_based_state[row_idx][col_idx]] = 1
            new_row.append(new_cell)
        one_hot_encoded_state.append(new_row)
...
def generate_dataset_for_dt_newest_one_hot_encoded(num_episodes=10, path_to_goal_maps_dir=f"{PROJECT_ROOT}/goal_maps/expert_zelda_lg"):
    kwargs = {
        'render_rank': 0,
        'render': False,
        'cropped_size': 22,
    ...
    for i in tqdm.tqdm(range(num_episodes)):
        ...
        # for _ in range(ep_len):
            ...
            current_state = pcgrl_env._rep._map
            current_state_oh = transform_state(current_state, x, y, obs_size)
            current_state_oh = convert_state_to_onehot_encoded(current_state_oh).reshape(-1)
            
            current_state_ohs.append(current_state_oh)
            # print(current_state_ohs)
            # get the target action from the goal map
            target_action = goal_map[y, x]
            target_action_oh = transform_action(target_action, action_dim[0])
            # import pdb; pdb.set_trace()
...
import pickle
paths = generate_dataset_for_dt_newest_one_hot_encoded(num_episodes=100)
